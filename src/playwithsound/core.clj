(ns playwithsound.core
	(:import
		[javax.sound.sampled AudioSystem DataLine$Info SourceDataLine AudioFormat AudioFormat$Encoding]))
...
(defn play-data [{:keys [line data playing] :as state} agent]
	(when (and line data playing)
		(.write #^SourceDataLine line data 0 (count data))
...
(def line (line-agent (open-line popular-format) 60))

(defn -main[&args]
  (play-melody line 70 400 mountain-king))
